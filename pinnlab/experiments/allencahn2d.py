...
class AllenCahn2D(BaseExperiment):
    """
    Allen–Cahn (2D, time-dependent):
        PDE: u_t - ε^2 (u_xx + u_yy) + (u^3 - u) = f(x,y,t)
    ...
    def pde_residual_loss(self, model, batch) -> torch.Tensor:
        ...
        r = u_t - ε^2 (u_xx + u_yy) + (u^3 - u) - f(x,y,t) = 0
        ...
        R = Ut - self.eps ** 2 * (Uxx + Uyy) + (C ** 3 - C) - Fc
        R2 = (R * MOK) ** 2
    ...
    def make_video(
        ...
        filename="allencahn2d.mp4", nt_video=60, fps=10,
